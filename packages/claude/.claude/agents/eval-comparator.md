...
description: "evalの2つの出力をブラインドで比較するエージェント。output_a_path、output_b_path、eval_promptを受け取り、comparison.jsonを出力する。eval-workflow内でのみ使用する。"
...
```json
{
  ...
  "output_quality": {
    "A": {"score": 9, "strengths": ["Complete solution"], "weaknesses": ["Minor header inconsistency"]},
...
`expectation_results` は expectations が提供された場合のみ出力する。
